...
# Plot with debugging info
  ggplot(data = seroconversion_data %>% filter(!is.na(calf_sex)),  # Remove NAs
         aes(x = Seroconversion_Week, y = Bacteria, color = calf_sex, group = calf_sex)) +
...
#####
  ...
  # Step 2: Filter and summarize seroconversion data
...
# 4. Count M and F per seroconversion week + Compute M/F Ratio
...
# 5. Plot: Seroconversion timing for each calf
...
# 6. Plot: Male-to-Female Seroconversion Ratio Over Time
